...
public sealed class GuardValueExtensionsIntTests
{
    ...
    public void Between_ValidRange_NoException(int value, int min, int max, bool excludeMin, bool excludeMax)
    ...
    public void Between_InvalidRange_ThrowException(int value, int min, int max, bool excludeMin, bool excludeMax)
        => Assert.Throws<GuardException>(()
            => Guard.Value(value).Between(min, max, excludeMin, excludeMax));
}